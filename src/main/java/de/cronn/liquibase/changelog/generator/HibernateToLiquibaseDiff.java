...
public abstract class HibernateToLiquibaseDiff {
  ...
  public String generateDiff(
      Class<? extends AbstractHibernatePopulatedConfig> hibernatePopulatedConfigClass,
      Class<? extends AbstractLiquibasePopulatedConfig> liquibasePopulatedConfigClass) {
    try (ConfigurableApplicationContext hibernatePopulatedContext =
            new AnnotationConfigApplicationContext(hibernatePopulatedConfigClass);
        Connection hibernatePopulatedConnection = getConnection(hibernatePopulatedContext);
        ConfigurableApplicationContext liquibasePopulatedContext =
            new AnnotationConfigApplicationContext(liquibasePopulatedConfigClass);
        Connection liquibasePopulatedConnection = getConnection(liquibasePopulatedContext)) {
      DiffResult diffResult =
          generateDiff(hibernatePopulatedConnection, liquibasePopulatedConnection);
  ...
  protected DiffResult generateDiff(Connection reference, Connection target) throws Exception {
    ...
    DiffResult diff =
        DiffGeneratorFactory.getInstance()
            .compare(referenceDatabase, targetDatabase, CompareControl.STANDARD);
  ...
  protected DiffResult filterDiffResult(DiffResult diffResult) {
    ...
    DiffResult result =
        new DiffResult(referenceDatabaseSnapshot, comparisonDatabaseSnapshot, compareControl);
  ...
  protected void handleChangedObject(
      DiffResult result, DatabaseObject obj, ObjectDifferences differences) {
